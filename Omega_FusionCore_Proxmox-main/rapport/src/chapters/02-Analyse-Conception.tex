...
Le développement de FusionCore s'inscrit dans le cadre global du projet académique \textbf{Grid-One}, réunissant quatre équipes synergiques (ATLAS pour l'infrastructure, OMEGA pour la mutualisation, SIGMA pour le portail, et ORION pour le calcul distribué). La méthodologie employée a été dictée par la nécessité d'une intégration continue entre ces sous-projets, ainsi que par les exigences de performance et de stabilité d'un ordonnancement système.

\paragraph{Démarche de prototypage itératif.}
La conception a suivi un cycle itératif inspiré des méthodes agiles. Nous avons débuté par une preuve de concept (PoC) validant l'interception des défauts de page via \texttt{userfaultfd} sur une VM de test minimale. Cette étape a permis de qualifier le risque technique essentiel. Les itérations suivantes ont successivement intégré la sérialisation binaire, le chiffrement TLS, et l'orchestration des migrations. Des revues de code régulières et des tests croisés avec les autres équipes Grid-One ont rythmé ces cycles.

\paragraph{Choix technologiques et outillage.}
La stricte séparation des préoccupations a guidé les choix logiciels~:
\begin{enumerate}
    \item \textbf{Plan de données} : \textbf{Rust} a été sélectionné pour l'implémentation des démons (\texttt{omega-daemon}). Son typage fort, couplé à \textit{Tokio} pour l'ordonnancement asynchrone, offre des pénalités d'exécution très faibles et élimine formellement les risques de \textit{data races} sur l'état partagé (cf. \texttt{DashMap} et \texttt{RwLock}).
    \item \textbf{Plan de contrôle} : \textbf{Python} s'est imposé pour le contrôleur global (\texttt{omega-controller}) du fait de sa flexibilité dans l'intégration avec l'API REST de Proxmox (\texttt{proxmoxer}) et de la rapidité d'implémentation des algorithmes de scoring d'admission.
    \item \textbf{Plateforme cible} : \textbf{Proxmox VE} adossé à un stockage distribué \textbf{Ceph RBD}, permettant une isolation saine des responsabilités entre la migration mémoire et le transfert disque, ce dernier étant délégué au stockage.
\end{enumerate}

\paragraph{Environnement de test.}
Le projet exploite l'outil \texttt{omega-lab.sh}, une infrastructure-as-code fournissant un lab de 3~nœuds interconnectés et une CI (Intégration Continue) complète s'appuyant sur \textit{pytest} (289~tests) pour le contrôleur et \textit{cargo test} (244~tests) pour les bibliothèques Rust.
...
Proxmox VE est une plateforme de virtualisation open source fondee sur QEMU/KVM qui gere les machines virtuelles (VMs) de maniere independante sur chaque nœud physique du cluster~\cite{proxmox_doc,qemu_doc,kvm_doc}. La RAM de chaque nœud constitue un silo isole : lorsqu'un nœud A utilise 90\,\% de sa memoire tandis que les nœuds B et C n'en consomment que 30\,\%, Proxmox refuse la creation de nouvelles VMs sur A --- meme si le cluster dispose collectivement de memoire disponible.

Ce cloisonnement entraine trois inefficacites mesurables :

\begin{enumerate}[label=(\roman*)]
    \item \textbf{Sur-provisionnement statique.} Une VM configuree avec 8~Gio de RAM ne les utilise pas en permanence. En pratique, une partie significative de sa memoire est «~froide » : des pages rarement accedees qui occupent de la RAM physique sans etre utiles a court terme.
    \item \textbf{Fragmentation inter-nœuds.} La somme des RAM disponibles sur les nœuds peut depasser le seuil de creation d'une VM, mais aucun nœud individuel ne dispose de la capacite suffisante.
...
L'analyse du comportement reel des VMs sur un cluster Proxmox de trois nœuds identifie quatre categories de ressources dont la mutualisation apporte un gain mesurable :
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.15\linewidth} p{0.30\linewidth} p{0.40\linewidth}}
        ...
        vCPU & VMs idle bloquant des cœurs physiques & cgroups v2 (\texttt{cpu.max}, \texttt{cpu.weight}), QMP hotplug \\
...
\begin{itemize}
    ...
    \item \textbf{Plan de contrôle (Python).} Les decisions de politique du cluster --- admission, placement topologique, migrations --- sont implementees en Python. La flexibilite du langage facilite l'integration avec l'API REST de Proxmox et permet des iterations rapides sur les algorithmes de placement.
\end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.20\linewidth} p{0.33\linewidth} p{0.33\linewidth}}
        ...
        Rust + Python (retenu) & Latence deterministe pour les donnees, flexibilite pour le contrôle & Complexite d'integration (bridge JSON via sous-processus) \\
        \bottomrule
    \end{tabular}
\end{table}
...
% -----------------------------------------------------------------------------
\subsection{Topologie du cluster et rôles symetriques}
\label{subsec:topologie-cluster}
% -----------------------------------------------------------------------------

Chaque nœud du cluster execute le meme binaire \texttt{omega-daemon} et joue simultanement deux rôles :

\begin{itemize}
    \item \textbf{Compute} : il heberge des VMs QEMU/KVM dont les pages froides peuvent etre evincees vers les autres nœuds.
    \item \textbf{Store} : il accepte et stocke en RAM les pages distantes envoyees par les VMs des autres nœuds.
\end{itemize}

L'architecture combine un modele peer-to-peer pour le canal de paging (port 9100) et un modele centralise pour les decisions de politique (contrôleur Python sur un seul nœud). La figure~\ref{fig:architecture-cluster} illustre cette topologie.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Nodes
        \node[node_box] (A) at (0, 0) {\textbf{Nœud A}\\omega-daemon\\VMs + Store};
        \node[node_box] (B) at (5, 0) {\textbf{Nœud B}\\omega-daemon\\VMs + Store};
        \node[node_box] (C) at (10, 0) {\textbf{Nœud C}\\omega-daemon\\VMs + Store};
    ...
    \end{tikzpicture}
    \caption{Topologie du cluster omega-remote-paging : paging peer-to-peer (TCP+TLS port~9100) et contrôle centralise (HTTP ports~9200/9300).}
    \label{fig:architecture-cluster}
\end{figure}

\subsection{Détail des interactions entre composants}
\label{subsec:interactions-composants}

La \autoref{fig:detailed-arch} présente une vue microscopique des interactions au sein d'un nœud et à l'échelle du cluster. Elle met en évidence le rôle central du \texttt{NodeState} partagé et les différents vecteurs de contrôle (QMP, UFFD, HTTP).

\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[
        scale=0.85, every node/.style={scale=0.85},
        comp/.style={draw, thick, rounded corners=2pt, fill=blue!5, minimum width=2.8cm, minimum height=0.8cm, align=center, font=\small},
        task/.style={draw, dashed, rounded corners=1pt, fill=white, minimum width=2.2cm, minimum height=0.6cm, align=center, font=\scriptsize},
        ext/.style={draw, thick, fill=gray!10, minimum width=2.5cm, minimum height=0.6cm, align=center, font=\small},
        container/.style={draw, thick, dotted, inner sep=10pt, fill=gray!2},
        flow/.style={-{Stealth[length=2.5mm]}, thick, draw=blue!70},
        ctrlflow/.style={-{Stealth[length=2.5mm]}, thick, dashed, draw=red!70},
        node_box/.style={draw, thick, fill=blue!2, inner sep=15pt}
    ]

        % --- NODE A (EXEC) ---
        \node[node_box] (NodeA) at (0, 0) [minimum width=6cm, minimum height=9cm] {};
        \node[above] at (NodeA.north) {\textbf{Nœud A (Exécution)}};

        \node[comp, fill=orange!10] (VM) at (0, 3.5) {\textbf{Machine Virtuelle}\\(QEMU/KVM)};
        \node[task, fill=orange!5] (Bridge) at (0, 2.7) {\texttt{uffd-bridge.so}};

        \node[draw, thick, fill=white, inner sep=8pt] (DaemonA) at (0, -0.5) [minimum width=5cm, minimum height=4.5cm] {};
        \node[below right] at (DaemonA.north west) {\texttt{omega-daemon} (Rust)};

        \node[task] (UFFD) at (0, 0.8) {UFFD Handler};
        \node[task] (CLOCK) at (0, 0.1) {CLOCK Engine};
        \node[task] (VCPU) at (0, -0.6) {VCPU Scheduler};
        \node[task] (GPU) at (0, -1.3) {GPU Multiplexer};
        \node[task, fill=blue!10] (State) at (0, -2.1) {\textbf{NodeState} (Shared)};

        % --- NODE B (STORE) ---
        \node[node_box] (NodeB) at (8, 0) [minimum width=3.5cm, minimum height=4cm] {};
        \node[above] at (NodeB.north) {\textbf{Nœud B (Store)}};
        \node[comp, fill=white] (StoreB) at (8, 0) {\texttt{omega-daemon}\\\textbf{PageStore} (RAM)};

        % --- CONTROLLER ---
        \node[comp, fill=green!10, minimum width=8cm] (Ctrl) at (4, -6.5) {\textbf{omega-controller} (Python)\\Admission $\cdot$ Migration $\cdot$ Ordonnancement};
        \node[ext] (Proxmox) at (4, -8) {Proxmox VE API / \texttt{qm}};

        % --- INTERACTIONS ---
        % Memory / Paging
        \draw[flow] (VM) -- node[right, font=\tiny] {Page Access} (0, 4.5);
        \draw[flow] (Bridge) -- node[left, font=\tiny] {UFFD Socket} (UFFD);
        \draw[flow] (UFFD) -- node[above, font=\tiny, sloped] {GET\_PAGE (TLS)} (StoreB);
        \draw[flow] (CLOCK) -- node[below, font=\tiny, sloped] {PUT\_PAGE (TLS)} (StoreB);

        % Control Node A
        \draw[ctrlflow] (Ctrl.north -| DaemonA) -- node[left, font=\tiny] {HTTP /api/status} (DaemonA.south);
        \draw[ctrlflow] (VCPU.west) -- ++(-1, 0) |- node[near start, left, font=\tiny] {QMP Hotplug} (VM.west);

        % Control Node B
        \draw[ctrlflow] (Ctrl.north -| StoreB) -- node[right, font=\tiny] {HTTP /control} (NodeB.south);

        % Proxmox
        \draw[ctrlflow] (Ctrl) -- (Proxmox);
        \draw[ctrlflow] (Proxmox) -- ++(-4, 0) |- node[left, font=\tiny, xshift=-1mm, yshift=2cm] {\texttt{qm migrate}} (NodeA.west);

        % Shared State
        \draw[flow, dashed, gray] (UFFD) -- (State);
        \draw[flow, dashed, gray] (CLOCK) -- (State);
        \draw[flow, dashed, gray] (VCPU) -- (State);
        \draw[flow, dashed, gray] (GPU) -- (State);

    \end{tikzpicture}
    \caption{Architecture détaillée et interactions : flux de données (bleu) et flux de contrôle (rouge). Le \texttt{NodeState} partagé asynchronise les décisions locales de l'orchestration globale.}
...
\begin{table}[htbp]
    \centering
    \caption{Ports reseau du systeme et leurs rôles.}
    \label{tab:ports-reseau}
    \begin{tabular}{c c p{0.55\linewidth}}
        \toprule
        \textbf{Port} & \textbf{Protocole} & \textbf{R\^ole} \\
        \midrule
        9100 & TCP + TLS & Store inter-nœuds : protocole binaire (header 20~octets + payload 4~Kio) \\
        9200 & HTTP JSON & API cluster : etat du nœud expose au contrôleur \\
        9300 & HTTP JSON & Canal de contrôle local : quotas, hotplug vCPU, metriques Prometheus \\
        \bottomrule
    \end{tabular}
\end{table}
...
Le daemon unifie \texttt{omega-daemon} lance huit tâches Tokio au demarrage (fichier \texttt{omega-daemon/src/main.rs}, lignes 57--448). Toutes ces tâches partagent un etat central de type \texttt{Arc<NodeState>} (fichier \texttt{omega-daemon/src/node\_state.rs}, ligne~94), qui contient les sous-systemes suivants sans verrou global :

\begin{itemize}
    \item \texttt{store: Arc<PageStore>} --- index \texttt{DashMap} shardé pour le stockage des pages distantes.
...
La communication entre tâches utilise trois mecanismes distincts, choisis en fonction du pattern d'acces :
...
La section suivante decrit en detail le mecanisme de paging distant qui constitue le cœur fonctionnel du systeme.
...
L'interface \texttt{userfaultfd} (User Fault File Descriptor) est un mecanisme du noyau Linux introduit a partir de la version 4.3 qui permet a un programme en espace utilisateur d'intercepter et de resoudre les defauts de page~\cite{userfaultfd_man}. Contrairement au swap Linux classique, qui implique un fichier ou une partition sur disque et un code noyau non modifiable, \texttt{userfaultfd} offre un contrôle total depuis l'espace utilisateur.

Le choix de \texttt{userfaultfd} plut\^ot que le swap Linux repose sur trois arguments :

\begin{enumerate}
    \item \textbf{Pas de disque.} Les pages froides sont envoyees vers la RAM d'un autre nœud, pas sur un support de stockage persistant. Le swap Linux necessite obligatoirement un fichier ou une partition.
...
Lorsque la pression memoire du nœud depasse un seuil configurable, le moteur d'eviction selectionne les pages les moins recemment utilisees pour les envoyer a un store distant. L'algorithme retenu est CLOCK (aussi appele \textit{second chance}), une approximation de LRU (Least Recently Used) a faible coût, largement utilise dans les noyaux de systemes d'exploitation~\cite{tanenbaum_os}.
...
\begin{enumerate}
    ...
    \item \textbf{Identification des VMs candidates.} Le \texttt{VmTracker} fournit la liste des VMs en etat \texttt{Running} sur le nœud local (fichier \texttt{eviction\_engine.rs}, lignes~152--156).
    ...
    \item \textbf{Envoi au store distant.} La page est envoyee via \texttt{PUT\_PAGE} TCP au nœud le moins charge, choisi par \texttt{hash(vm\_id, page\_id) \% num\_stores}.
    \item \textbf{Liberation locale.} L'appel \texttt{madvise(MADV\_DONTNEED)} libere la page du page table local apres envoi.
\end{enumerate}

L'algorithme~\ref{alg:clock-eviction} décrit plus formellement ce mécanisme itératif de sélection des pages à distance.

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwData{VM $v$, Bit de référence $B(p)$ pour chaque page $p \in v$}
    \KwResult{Sélection d'une page $p$ pour éviction}

    \While{vraie}{
        $p \leftarrow \text{page pointée par l'aiguille CLOCK de } v$\;
        \If{$B(p) = 1$}{
            $B(p) \leftarrow 0$\;
            Avancer l'aiguille CLOCK vers la page suivante de $v$\;
        }
        \Else{
            \Return $p$ (Page sélectionnée)\;
        }
    }
    \caption{Sélection de page froide par approximation CLOCK}
    \label{alg:clock-eviction}
\end{algorithm}
...
Si le taux de recuperation reste eleve malgre le ralentissement de l'eviction, le daemon genere des recommandations de migration via la \texttt{MigrationPolicy} locale, signalant au contrôleur Python que la VM devrait etre deplacee vers un nœud moins charge.

Le comportement de l'intervalle est régi par la fonction suivante :
\begin{equation}
    \label{eq:adaptive-interval}
    I_{check} =
    \begin{cases}
        I_{accel} = 2\,\text{s} & \text{si } F_{\text{remote}}^t > 5.0\,\text{page/s} \\
        I_{nom} = 10\,\text{s} & \text{sinon}
    \end{cases}
\end{equation}
o\`u $F_{\text{remote}}^t$ est la moyenne glissante du taux de fautes distants servies.

% -----------------------------------------------------------------------------
\subsection{Le protocole TCP binaire inter-nœuds}
\label{subsec:protocole-tcp}
% -----------------------------------------------------------------------------

Les echanges de pages entre nœuds utilisent un protocole binaire proprietaire sur TCP, concu pour minimiser la latence de parsing. Chaque trame comporte un en-tete fixe de 20~octets suivi d'un payload variable. Le format est defini dans le fichier \texttt{docs/protocol.md} et implemente dans \texttt{node-bc-store/src/protocol.rs} :
...
Tous les champs multi-octets sont en big-endian. Le champ \texttt{magic} (\texttt{0x524D} pour «~RM » --- \textit{Remote Memory}) identifie le protocole. Le champ \texttt{flags} reserve le bit~0 pour la compression LZ4~\cite{lz4_compression} (non activee par defaut).
...
La distribution des pages entre stores est deterministe : \texttt{hash(vm\_id, page\_id) \% num\_stores}. Ce schema evite toute coordination inter-nœuds pour localiser une page. Le facteur de replication est configurable (defaut : 2 copies via la variable d'environnement \texttt{OMEGA\_REPLICATION\_FACTOR}).
...
Le lien avec le systeme de quotas est direct : lorsque le balloon reduit la RAM physique du guest a \texttt{balloon\_actual\_mib}, le budget distant peut croître dynamiquement :
...
\begin{enumerate}
    \item \textbf{Capacite totale.} La somme des RAM disponibles sur tous les nœuds du cluster doit etre superieure ou egale a \texttt{vm.max\_mem}.
    \item \textbf{Selection du nœud cible.} Le moteur de placement topologique (fichier \texttt{topology\_placement.py}) calcule un score multi-criteres pour chaque nœud candidat :
    \begin{equation}
    ...
    \item \textbf{Veto.} Si aucun nœud ne peut accueillir la VM sans dépasser 80\,\% de RAM utilisée, l'admission est refusée.
\end{enumerate}

L'algorithme~\ref{alg:admission} formalise ce processus, incluant le calcul des budgets locaux et distants.

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwData{VM $v$ à admettre, Cluster $\mathcal{N}$, Marge de sécurité $\alpha$}
    \KwResult{Décision d'admission (nœud, $M_{\ell}(v)$, $M_r(v)$)}

    $Candidates \leftarrow \{n \in \mathcal{N} \mid n \text{ est joignable}\}$\;
    $TotalFree \leftarrow \sum_{n \in Candidates} R_{\text{libre}}(n) \times (1 - \alpha)$\;

    \If{$TotalFree < M_{\max}(v)$}{
        \Return{\texttt{REFUS} (Capacité insuffisante)}\;
    }

    $n^* \leftarrow \text{argmax}_{n \in Candidates} (S(v, n))$\;
    \If{$R_{\text{libre}}(n^*, t) \times (1 - \alpha) < 0.2 \times M_{\max}(v)$}{
        \Return{\texttt{REFUS} (Pas assez de RAM locale sur le meilleur nœud)}\;
    }

    $M_{\ell}(v) \leftarrow \min(M_{\max}(v), R_{\text{libre}}(n^*, t) \times (1 - \alpha))$\;
    $M_r(v) \leftarrow M_{\max}(v) - M_{\ell}(v)$\;

    \If{$M_r(v) > 0$}{
        $OtherNodes \leftarrow Candidates \setminus \{n^*\}$\;
        $RemoteAvail \leftarrow \sum_{n \in OtherNodes} R_{\text{libre}}(n) \times (1 - \alpha)$\;
        \If{$RemoteAvail < M_r(v)$}{
            \Return{\texttt{REFUS} (RAM distante insuffisante)}\;
        }
    }

    \Return{\texttt{ADMIS} ($n^*, M_{\ell}(v), M_r(v)$)}\;
    \caption{Admission et budgétisation d'une VM}
    \label{alg:admission}
\end{algorithm}
...
L'admission des ressources CPU est geree par le \texttt{ClusterVcpuPoolPlanner} (fichier \texttt{controller/controller/cpu\_admission.py}). Il selectionne le nœud avec le plus de slots vCPU libres et verifie que l'overcommit ne depasse pas $3\times$ le nombre de cœurs physiques.

L'admission GPU est geree par le \texttt{GpuAdmissionController} (fichier \texttt{controller/controller/gpu\_admission.py}). Pour chaque VM necessitant un GPU, il verifie que le nœud cible dispose de suffisamment de VRAM libre :

\begin{equation}
    \texttt{gpu\_free\_vram\_mib} \geq \texttt{vm.gpu\_budget\_mib}
\end{equation}

Si aucun nœud GPU n'a assez de VRAM, l'admission est refusee. Ces trois contrôles (RAM, vCPU, GPU) sont evalues conjointement : la VM n'est admise que si les trois sont satisfaits.
...
\begin{itemize}
    \item \texttt{VCPU\_PER\_PCPU = 3} : chaque cœur physique expose 3 slots virtuels.
...
Le scheduler ne decide jamais d'une migration de maniere autonome. Il expose les signaux de pression via le snapshot \texttt{NodeStatus} (fichier \texttt{node\_state.rs}), que le contrôleur Python interroge periodiquement via \texttt{GET /api/status}. Deux conditions declenchent l'escalade :
...
Le contrôleur Python evalue alors la migration en tenant compte de l'ensemble des ressources du cluster (section~\ref{sec:migration}).
...
\begin{enumerate}
    \item La pression I/O du nœud depasse \texttt{DISK\_PRESSURE\_TRIGGER\_PCT = 5.0\,\%} (PSI \texttt{some.avg10}).
...
Si \texttt{io.weight} n'est pas disponible dans le cgroup de la VM (backend non supporte), le systeme effectue un fallback gracieux : la methode \texttt{mark\_io\_control\_unsupported()} desactive le contrôle pour cette VM et consigne la raison dans les logs (fichier \texttt{main.rs}, lignes~979--1004). La telemetrie et la migration restent actives.
...
Ces valeurs alimentent le scheduler disque et sont exposees dans le snapshot \texttt{VmStatusEntry} (fichier \texttt{node\_state.rs}, lignes~68--70), rendant les debits I/O par VM visibles au contrôleur Python pour les decisions de migration.
...
Dans un cluster Proxmox standard, un GPU physique ne peut etre assigne qu'a une seule VM via le mecanisme de PCI passthrough. Cette exclusivite est problématique lorsque plusieurs VMs necessite des capacites GPU (inference IA, rendu graphique) mais qu'aucune n'utilise le GPU a pleine capacite en permanence.
...
\begin{itemize}
    ...
    \item \textbf{vGPU (NVIDIA GRID/MIG)} : solution proprietaire necessitant des licences coûteuses.
...
Le multiplexeur (fichier \texttt{omega-daemon/src/gpu\_multiplexer.rs}) ecoute sur un socket Unix (\texttt{/run/omega/gpu.sock}) et gere les requetes GPU de toutes les VMs du nœud. Son architecture interne repose sur trois composants :
...
Le nombre magique est \texttt{0x4F4D4750} («~OMGP » pour \textit{Omega GPU Protocol}, fichier \texttt{gpu\_protocol.rs}, ligne~32). Les types de messages sont definis par l'enumeration \texttt{MsgType} (lignes~39--49) :
...
La correlation requete/reponse est assuree par le champ \texttt{seq} : le multiplexeur maintient un \texttt{HashMap<(vm\_id, seq), oneshot::Sender>} pour router chaque reponse vers la tâche Tokio correspondante.
...
\begin{itemize}
    \item La \textbf{capacite du nœud} provient du backend DRM reel.
    \item Les \textbf{budgets des VMs} proviennent de la configuration Proxmox (champs \texttt{description} et \texttt{tags}).
    \item Le contrôleur Python ne reinvente pas ces valeurs : il consomme ce que le daemon publie via \texttt{/control/gpu/status} et \texttt{/api/status}.
\end{itemize}
...
La politique de migration est implementee dans deux modules complementaires : \texttt{vm\_migration.rs} (daemon Rust, lignes 1--17) et \texttt{migration\_policy.py} (contrôleur Python). La matrice de decision est la suivante :
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.25\linewidth} p{0.25\linewidth} c}
        \toprule
        \textbf{Etat de la VM} & \textbf{Pression nœud} & \textbf{Type} \\
        \midrule
...
\begin{itemize}
    ...
    \item \texttt{vcpu\_saturation\_pct = 90.0} : nœud $>$ 90\,\% vCPU utilises.
...
Les raisons de migration sont enumerees par \texttt{MigrationReason} (fichier \texttt{vm\_migration.rs}, ligne~64) : \texttt{MemoryPressure}, \texttt{CpuSaturation}, \texttt{ExcessiveRemotePaging}, \texttt{MaintenanceDrain}, \texttt{AdminRequest}. Le contrôleur Python ajoute \texttt{DiskSaturation} et \texttt{GpuSaturation} (fichier \texttt{migration\_policy.py}, lignes~50--57).

% -----------------------------------------------------------------------------
\subsection{Selection du nœud cible}
\label{subsec:selection-cible}
% -----------------------------------------------------------------------------

Le choix du nœud cible utilise un score composite calcule par la methode \texttt{\_placement\_score\_for\_vm()} (fichier \texttt{migration\_policy.py}, lignes~534--556). Deux cas sont distingues :
...
o\`u $r_{\text{conso}} = \min(\text{local\_vms}, 12) / 12$ penalise la consolidation excessive. Un veto absolu empeche l'envoi vers un nœud qui, apres accueil de la VM, descendrait sous 20\,\% de RAM libre (methode \texttt{can\_accept\_vm()}, ligne~103).

L'algorithme~\ref{alg:migration-selection} résume la logique globale de décision de migration prise par le contrôleur.

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwData{Cluster $\mathcal{N}$, Ensemble des VMs $\mathcal{V}_t$, Seuils $\mathcal{T}$}
    \KwResult{Liste des migrations recommandées $\mathcal{M}$}

    $\mathcal{M} \leftarrow \emptyset$\;
    \For{chaque nœud $n \in \mathcal{N}$}{
        \For{chaque VM $v$ sur $n$}{
            $Type, Raison \leftarrow \text{EvaluerRegles}(v, n, \mathcal{T})$\;
            \If{$Raison \neq \emptyset$}{
                $Eligibles \leftarrow \{n' \in \mathcal{N} \mid n' \neq n \land \text{PeutAccueillir}(n', v)\}$\;
                \If{$Eligibles \neq \emptyset$}{
                    $n^* \leftarrow \text{argmax}_{n' \in Eligibles} (\text{ScorePlacement}(n', v))$\;
                    $\mathcal{M} \leftarrow \mathcal{M} \cup \{(v, n, n^*, Type, Raison)\}$\;
                }
            }
        }
    }
    Trier $\mathcal{M}$ par urgence décroissante\;
    \Return $\mathcal{M}$ filtré par quota de migrations concurrentes\;
    \caption{Évaluation et sélection des migrations cluster-wide}
    \label{alg:migration-selection}
\end{algorithm}
...
\begin{enumerate}
    ...
    \item \textbf{Plan global (contrôleur Python).} La \texttt{MigrationPolicy} Python (fichier \texttt{migration\_policy.py}, methode \texttt{evaluate()}, ligne~230) analyse l'etat complet du cluster et peut declencher plusieurs migrations simultanees. Elle deduplique les candidats (une VM ne migre qu'une fois) et les trie par urgence decroissante.
\end{enumerate}

Cette architecture a double plan garantit que les migrations urgentes (pression locale critique) sont traitees rapidement par le daemon, tandis que le contrôleur Python optimise les migrations a l'echelle du cluster avec une vue globale.
...
\begin{enumerate}
    ...
    \item \textbf{Circuit-breaker par nœud} (lignes~63--72) :
    \begin{itemize}
    ...
    \item \textbf{Cache du dernier etat connu} : si un nœud est injoignable, son dernier etat connu est retourne avec un marqueur \texttt{stale=True}. Les donnees en cache restent valides pendant 120~secondes.
\end{enumerate}

Ce mecanisme garantit la contrainte C4 : un nœud inaccessible ne bloque pas le reste du cluster. La figure~\ref{fig:circuit-breaker} illustre la machine a etats du circuit-breaker.
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Machine a etats du circuit-breaker du \texttt{ResilientCollector}~\cite{nygard_release}. Un nœud inaccessible passe en etat \textsc{open} apres 3 echecs et ses dernieres donnees connues sont utilisees pendant la periode de recuperation.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.45\linewidth} c p{0.30\linewidth}}
        ...
        \midrule
        \texttt{omega\_vcpu\_slots\_total} & gauge & Slots vCPU totaux du nœud \\
...
\begin{itemize}
    ...
    \item \texttt{steal\_pct $<$ 10\,\%} sur les nœuds : pas de surcharge CPU.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.28\linewidth} c p{0.42\linewidth}}
        ...
        \midrule
        Store TCP (\texttt{:9100}) & Evenementielle & Accepte les connexions TLS inter-nœuds \\
        Cluster API (\texttt{:9200}) & Evenementielle & Expose l'etat au contrôleur Python \\
...
La gestion d'erreur est exhaustive : si QEMU n'accepte plus de vCPU supplementaire (limite de la configuration \texttt{maxcpus}), le scheduler ne tente pas de nouvel hotplug et remonte directement la decision de migration au contrôleur Python via l'API de statut (\texttt{GET /api/status}).

% -----------------------------------------------------------------------------
\subsection{Evaluation de la politique de migration par le contrôleur Python}
\label{subsec:impl-migration-policy}
% -----------------------------------------------------------------------------

La methode \texttt{evaluate()} de la \texttt{MigrationPolicy} Python s'execute a chaque cycle du contrôleur (configurable, par defaut toutes les 30~s) et suit un algorithme en cinq etapes :

\begin{enumerate}
    \item \textbf{Collecte resiliente.} Le \texttt{ResilientCollector}~\cite{nygard_release} interroge chaque nœud via \texttt{GET /api/status} avec retry exponentiel. Les nœuds injoignables retournent leur dernier etat cache.
    \item \textbf{Identification des VMs candidates.} Pour chaque VM sur chaque nœud, les regles de la table~\ref{tab:migration-matrice} sont evaluees. Une raison de migration est associee si l'une des conditions est satisfaite.
    \item \textbf{Selection du nœud cible.} Pour chaque VM candidate, le score de placement (equations~\ref{eq:score-sans-gpu} et~\ref{eq:score-avec-gpu}) est calcule pour chaque nœud candidat. Le veto de capacite (\texttt{can\_accept\_vm()}) empeche le sur-chargement du nœud cible.
...
\textbf{Scenario : nœud A surchargé --- cycle complet de reequilibrage.}

\begin{enumerate}
    \item \textbf{Detection de la pression memoire.} La fonction \texttt{read\_meminfo()} detecte que l'usage RAM du nœud A depasse 85\,\% (\texttt{evict\_threshold\_pct}).

    \item \textbf{Eviction CLOCK.} L'\texttt{EvictionEngine} selectionne les pages froides des VMs locales et les envoie via \texttt{PUT\_PAGE} TCP+TLS aux stores des nœuds B et C. Le \texttt{QuotaRegistry} verifie, pour chaque page, que le budget distant n'est pas depasse (equation~\ref{eq:check-put}).
    ...
    \item \textbf{Pression CPU simultanee.} Le \texttt{CgroupCpuMonitor} (1~ms) detecte que la VM~101 est throttlée a 35\,\% (\texttt{throttle\_ratio $>$ THROTTLE\_THRESHOLD}). Le \texttt{VcpuScheduler} tente un hotplug : aucun slot disponible. Decision : \texttt{VcpuDecision::MigrateRequired}.
    ...
    \item \textbf{Decision de migration (contrôleur Python).} Le \texttt{ResilientCollector} collecte l'etat des trois nœuds. La \texttt{MigrationPolicy} evalue le score de placement : le nœud C a le meilleur score ($r_{\text{RAM}} = 0{,}65$, $r_{\text{vCPU}} = 0{,}70$, $r_{\text{disk}} = 0{,}95$). La decision est : migration live de la VM~101 vers le nœud C.
    ...
    \item \textbf{Nettoyage post-migration.} Sur le nœud A :
    \begin{itemize}
    ...
    \end{itemize}
    Le nœud C enregistre automatiquement la VM~101 dans son \texttt{VcpuScheduler} et \texttt{QuotaRegistry} via le cycle de decouverte du \texttt{VmTracker} (intervalle 15~secondes).
\end{enumerate}
