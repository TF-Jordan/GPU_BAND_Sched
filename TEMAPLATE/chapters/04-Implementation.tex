...
Le cluster de référence est \emph{GRIDZERO}. Sa composition matérielle est synthétisée dans le tableau~\ref{tab:gridzero}. Quatre nœuds physiques sont raccordés en VLANs séparés (management 1~Gb/s, stockage Ceph 10/25~Gb/s SFP+, public 1~Gb/s). Un onduleur en amont assure la tolérance aux coupures du réseau électrique. La salle est instrumentée (température, humidité) par les capteurs IoT déployés par ATLAS.
...
CPU par nœud             & Intel Corei5-10600K (RAM, REM), Intel Corei5-9400F (EMILIA) \\
RAM par nœud             & 64 Gio (EMILIA, REM), 32 Gio pour RAM \\
Stockage local           & 4.08 TiB (EMILIA), 7.74 TiB (RAM) et 7.28 TiB REM \\
Pool Ceph                & RBD \\
GPU                      & NVIDIA RTX 3090 Ti (RAM, REM) et NVIDIA RTX 2080 Ti (EMILIA)\\
\bottomrule
\end{tabular}
.}
\label{tab:gridzero}
...
\midrule
Kernel Linux           & $\ge$ 6.17           & userfaultfd non privilégié \\
Proxmox VE             & 9.1            & Hyperviseur, cluster Corosync, Ceph \\
...
\centering
\small
\caption{Benchmarks de l'algorithme CLOCK (\texttt{node-a-agent}
\begin{tabular}{lrr}
\toprule
