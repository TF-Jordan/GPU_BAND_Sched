...
Ce chapitre presente l'analyse detaillee des contraintes du systeme, la conception de chaque sous-systeme, et les elements cles d'implementation du projet \textit{omega-remote-paging}. L'objectif est de justifier chaque choix architectural par rapport aux alternatives ecartees, en ancrant chaque decision dans les structures de donnees, les constantes et les invariants reels du code source. Le chapitre couvre successivement l'analyse du probleme (section~\ref{sec:analyse-probleme}), l'architecture generale (section~\ref{sec:architecture-generale}), puis chacun des six axes fonctionnels du systeme : paging distant~\cite{clark_2005}, quotas, scheduler vCPU, scheduler disque, multiplexeur GPU et migration automatique. La section~\ref{sec:implementation} detaille les choix d'implementation concrets avant la synthese finale (section~\ref{sec:synthese}).
...
La migration automatique est le mecanisme de dernier recours qui reequilibre le cluster lorsque les mecanismes locaux (eviction, partage CPU/disque) ne suffisent plus. Elle exploite les capacites natives de Proxmox (\texttt{qm migrate}) avec deux strategies : live (pre-copy KVM~\cite{clark_2005}) et cold.
...
% =============================================================================
\section{Implementation des composants cles}
\label{sec:implementation}
% =============================================================================

Cette section presente les choix d'implementation concrets des composants critiques du systeme, en se concentrant sur les patterns techniques qui assurent les proprietes de performance et de surete sans reproduire le code source. L'objectif est de documenter les decisions d'ingenierie et leur justification.
...
Ce chapitre a presente l'analyse detaillee des contraintes, la conception et les elements cles d'implementation du projet omega-remote-paging. Les six axes fonctionnels --- paging distant, quotas, scheduler vCPU, scheduler disque, multiplexeur GPU et migration automatique --- forment un systeme coherent dont les invariants formels (equations~\ref{eq:quota-invariant}, \ref{eq:quota-total}, \ref{eq:check-put}) garantissent la surete a l'execution. La section~\ref{sec:implementation} a montre comment les abstractions Tokio/DashMap se traduisent en un plan de donnees Rust performant, tandis que le contr\^oleur Python assure la politique de cluster avec le patron circuit-breaker~\cite{nygard_release}. Le chapitre suivant presentera les resultats experimentaux obtenus sur un cluster Proxmox reel et les lecons tirees de l'evaluation du systeme.
